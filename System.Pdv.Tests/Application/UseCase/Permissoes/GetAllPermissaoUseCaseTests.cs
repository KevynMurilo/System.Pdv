...
public class GetAllPermissaoUseCaseTests
{
    ...
    public async Task ExecuteAsync_ShouldReturnPermissions_WhenPermissionsExist()
    {
        ...
        _mockPermissaoRepository.Setup(p => p.GetAllAsync()).ReturnsAsync(permissoes);
    ...
    {
        _mockPermissaoRepository.Setup(p => p.GetAllAsync()).ReturnsAsync(new List<Permissao>());
    ...
    {
        _mockPermissaoRepository.Setup(p => p.GetAllAsync()).ThrowsAsync(new Exception("Database error"));
